...
class LandsGame():
  def __init__(self, num_elements=lv.NUM_ELEMENTS, num_cards=lv.NUM_CARDS, start_hand=lv.START_HAND, player_count = lv.PLAYER_COUNT):
    ...
    # AAA self.players = [{"field": np.zeros(self.num_elements, dtype=np.int8), "hand": np.zeros((self.num_elements), dtype=np.int8), "discard": np.zeros((self.num_elements), dtype=np.int8), "deck": None} for i in range(self.player_count)]
